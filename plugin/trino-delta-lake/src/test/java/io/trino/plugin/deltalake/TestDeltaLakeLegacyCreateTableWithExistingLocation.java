        ...
        extends AbstractTestQueryFramework
{
    ...
    {
        String tableName = "test_legacy_create_table_" + randomTableSuffix();
        ...
        assertQueryFails(format("CREATE TABLE %s.%s.%s (dummy int) with (location = '%s')", CATALOG_NAME, "tpch", tableNameNew, tableLocation),
        ...
        Session sessionWithLegacyCreateTableEnabled = Session
                ...
                .build();
        assertQuerySucceeds(sessionWithLegacyCreateTableEnabled, format("CREATE TABLE %s.%s.%s (dummy int) with (location = '%s')", CATALOG_NAME, "tpch", tableNameNew, tableLocation));

        assertQuery("SELECT * FROM " + tableName, "VALUES (1, 'INDIA', true)");

        metastore.dropTable("tpch", tableNameNew, false);
        assertThat(metastore.getTable("tpch", tableNameNew)).as("Table should be dropped from metastore").isEmpty();
