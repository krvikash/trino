        ...
        implements TransactionLogSynchronizer
{
    ...
    public void write(ConnectorSession session, String clusterId, Path newLogEntryPath, byte[] entryContents)
    {
        ...
        try {
            createStorageObjectExclusively(newLogEntryPath.toUri(), entryContents, pathCodec, storage);
        }
        catch (IOException e) {
            throw new UncheckedIOException(e);
        }
     ...
     */
    private static StorageObject createStorageObjectExclusively(URI blobPath, byte[] content, PathCodec pathCodec, Storage storage)
            throws IOException
    {
    ...
    /**
     * Helper for converting from a {@code Map<String, byte[]>} metadata map that may be in a
     * StorageObject into a {@code Map<String, String>} suitable for placement inside a
     * {@link GoogleCloudStorageItemInfo}.
     */
    ...
    {
        return bytes == null ? Data.NULL_STRING : Base64.getEncoder().encodeToString(bytes);
    }
}
