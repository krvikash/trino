        ...
        implements Provider<Procedure>
{
    ...
    private void doRegisterTable(ConnectorSession clientSession, String schemaName, String tableName, String tableLocation, Optional<String> metadataFileName)
    {
        ...
        if (catalog.tableExists(clientSession, schemaTableName)) {
        ...
        String metadataFileLocation = metadataFileName
                .map(name -> format("%s/%s/%s", stripTrailingSlash(tableLocation), METADATA_FOLDER_NAME, name))
                .orElseGet(() -> catalog.latestMetadataLocation(clientSession, tableLocation));

        catalog.registerTable(clientSession, schemaTableName, metadataFileLocation);
    }
}
