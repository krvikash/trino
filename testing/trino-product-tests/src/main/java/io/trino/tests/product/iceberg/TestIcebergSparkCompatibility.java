        ...
        extends ProductTest
{
            ...
            throws TException
    {
        ...
        List<Row> expectedDescribeTableRows = expectedDescribeTable.rows().stream()
                .map(columns -> row(columns.toArray()))
                .collect(toImmutableList());

        QueryResult expectedShowCreateTable = onTrino().executeQuery("SHOW CREATE TABLE " + trinoTableName);
        List<Row> expectedShowCreateTableRows = expectedShowCreateTable.rows().stream()
                .map(columns -> row(columns.toArray()))
                .collect(toImmutableList());
        ...
        assertThat(actualDescribeTable)
                .hasColumns(expectedDescribeTable.getColumnTypes())
                .containsExactlyInOrder(expectedDescribeTableRows);
        assertThat(actualShowCreateTable)
                .hasColumns(expectedShowCreateTable.getColumnTypes())
                .containsExactlyInOrder(expectedShowCreateTableRows);

        onSpark().executeQuery(format("INSERT INTO %s values(3, 'POLAND', true)", sparkTableName));
        List<Row> newExpected = List.of(row(1, "INDIA", true), row(2, "USA", false), row(3, "POLAND", true));

        assertThat(onTrino().executeQuery(format("SELECT * FROM %s", trinoTableName)))
                .containsOnly(newExpected);
        assertThat(onSpark().executeQuery(format("SELECT * FROM %s", sparkTableName)))
                .containsOnly(newExpected);
            ...
            throws TException
    {
        ...
        assertQueryFailure(() -> onTrino().executeQuery(format("CALL iceberg.system.register_table ('%s', '%s', '%s')",
                TEST_SCHEMA_NAME, baseTableName, tableLocation))).hasMessageMatching(".*No versioned metadata file exists at location.*");
